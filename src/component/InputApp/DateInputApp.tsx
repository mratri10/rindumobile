...
import { TouchableOpacity, Text, StyleSheet, View, Dimensions, Modal, ScrollView, StyleProp, ViewStyle, TextStyle } from 'react-native'
...
function DateInput({ value, onChange, placeholder }: DateType) {
    const [show, setShow] = useState(false)
    const [dateData, setDateData] = useState<DateItemType>(value)

    // Value awal jika user tidak memasukan defaultValue
    useEffect(() => {
        setDateData({
            day: 15,
            month: 6,
        ...
        <TouchableOpacity onPress={() => setShow(true)} >
            <View style={styles.main}>
                {value && value.day && value.month && value.year ?
                    <Text style={styles.textMain}>{must2Number(value.day.toString())} {converMonthName(value.month)} {value.year}</Text> :
                    <Text style={styles.textPlaceholder}>{placeholder ?? "Masukan Tanggal Anda"}</Text>}
            </View>
            <Modal visible={show} transparent={true}>
                <View style={styles.date}>
                    ...
                    <View style={styles.timeHeader}>
                        <ItemDate name='Month' data={bulan} onChange={(v) => convertMonth(v)} value={(value?.month ?? 6) - 1} />
                        <ItemDate name='Day' data={tanggal} onChange={(v) => convertDay(v)} value={(value?.day ?? 15) - 1} />
                        <ItemDate name='Year' data={tahun} onChange={(v) => convertYear(v)} value={(value?.year ?? 2010) - FirstYear} />
                    </View>
                    <View style={styles.contentBottom}>
                        ...
                        <TouchableOpacity style={styles.button} onPress={() => {
                            const defaultValue: DateItemType = { year: 2010, month: 6, day: 15 }
                            setShow(false)
...
const ItemDate = ({ name, data, value, onChange, }: ItemDateType) => {
    ...
    return (
        <View style={styles.coloumnTime}>
            ...
            <ScrollView style={styles.contentTime}
